...
#2019年7月24日
# Url  服务名
jenkins_home = '/data/jenkins/jenkins2/jobs/'
jenkins_user = 'root'
...
with open(jenkins_conf,'r') as f:
    for line in f.readlines():
        try:
            t = line.split(',')
            git_url = t[0]
            server_name = 'Pre_'+t[1]
            port = t[2].strip('\n')
            if os.path.exists(jenkins_home+server_name):
                print '%s Job Is Exist'%server_name
                continue
            ...
            new_job = jenkins_home + server_name
            print "GiT:", git_url, "服务名:", server_name, "端口号:", port, "新Job:", new_job
            result = os.system('/application/java/bin/java -jar /data/deploy/jenkins-cli.jar -s http://192.168.1.62:7000 -auth %s:%s  copy-job template %s'%(jenkins_user,jenkins_passwd,server_name) )
