...
#原nginx日志存放路径
LOG_PATH=/app/logs/nginx/
#nginx 切割后存放路径
LOG_FILES_DIR=/app/backup/$(date -d "yesterday" +"%Y")/$(date -d "yesterday" +"%m")

#待切割日志名称集合
LOG_FILES_NAME=($(ls /app/logs/nginx))

#nginx启动脚本
NGINX_SBIN="/app/software/nginx/nginx/sbin/nginx"
